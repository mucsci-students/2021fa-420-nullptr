...
void UMLData::changeParameterName(method_ptr methodIter, string oldParamName, string newParamName)
{
  ...
  if(methodIter->getParam().size() == 0)
  {
    throw std::runtime_error("You have no parameters.");
    return;
  }
  
  if (!isValidName(newParamName))
  {
    throw std::runtime_error("The new name you typed is not valid.");
    return;
  }
  ...
  for(auto param : methodIter->getParam())
  {
    if(param.getName() == oldParamName)
    {
      param.changeName(newParamName);
      return;
    }
  }
